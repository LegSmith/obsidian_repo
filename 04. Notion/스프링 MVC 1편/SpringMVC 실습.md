...
- **==타임리프는 th:xxx 가 붙은 부분은 서버사이드랜더링(SSR)이 되고, 기존 것을 대체==**한다. th:xxx 가 없으면 기존 html xxx속성이 그대로 사용된다.
- 서버를 가동하지 않고 HTML 파일을 열었을때 html 코드에 th:xxx 가 있어도 th는 무시하고 열기 때문에 **==순수 HTML 을 그대로 유지하면서 뷰 템플릿도 사용가능하다.==**
