...
그러면 **==TransactionRepository 가 Dao 를 상속받아서 동적쿼리를 사용할 수 있게 된다. →문법이다 !!==**  

즉, ==**Dao 인터페이스(동적쿼리 메서드) → TransactionRepositoryImpl(동적쿼리 구현) → TransactionRepository 가 Dao를 상속받는다.**==


- `findTransactionList` 를 `TransactionRepository` 에 생성하지 않은 이유는 **==스프링데이터 JPA 에서 @Query 로 동적쿼리를 생성할 수 없기 때문==**이다.
