...
- `**동작순서**`
    ...
    2. ==_**계좌번호,비밀번호가 들어있는 DTO로 중복된 계좌가 아닌지 조회**_==한다.
    3. 위 조건이 다맞으면 ==_**계좌를 등록하고 등록한 계좌를 DTO로 응답**_==해준다.
...
- 우선 `fullName`과 찾은 계좌목록을 필드로 가지고 ==_**계좌목록은 엔티티이므로 한번더 DTO로 변환**_==한다.
...
- 계좌삭제 로직은 ==_**계좌번호를 확인하여 유효한 계좌번호**_== 인지 확인 후 ==_**계좌번호 소유자**_==_==**를 확인**==_하고 확인이 되면 ==_**계좌를 삭제**_==한다.
...
- `depositAccountPS` 는 _==**영속성 컨텍스트에 저장됐기 때문에 영속화된 객체**==_이다. 그렇기 떄문에 ==**값을 수정하면 JPA의 더티체킹이 일어나서 Transaction 종료 후 DB에 update 쿼리가 나가게 된다.**==
