...
- `Service` 테스트는 _==**직접 DB와 연결없이 Mock 환경에서 테스트**==_하면 된다.
- 실제 `AccountService` 를 띄워놓고 `@Mock` 어노테이션으로 `가짜 UserRepository` 와 `가짜 AccountRepository` 를 `AccountService`에 주입한다.
- 그리고 Service 코드에서는 ==_**계좌등록을 했을때 반환하는 DTO가 제대로 DTO로 전환되는지 검증**_==한다.
...
- 여기서 문제는 하나의 `MockUser` 와 하나의 `MockAccount` 객체를 ==_**2개의 stub 에서 사용하면 데이터가 꼬여질 수 있다**_==.
