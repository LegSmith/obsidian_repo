...
- REST API 형식으로는 `/s/account/{id}` 로 하는게 맞다. 하지만 그렇게 되면 ==_**로그인한 유저가 타 유저의 계좌목록을 요청할 수 있기 때문에 권한검증 로직이 필요**_==해진다.
- 순수하게 ==_**컨트롤러에서는 계좌목록만 보여주는 로직만 작성하기 위해 로그인한 유저가 본인계좌만 조회할 수 있게 설계**_==하였다.
