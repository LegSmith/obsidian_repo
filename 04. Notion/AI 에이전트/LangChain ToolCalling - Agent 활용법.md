...
- _==**LLM의 한계(최신 정보 부족, 특정 작업 수행 불가 등등)를 극복**==_하는 방법이다
- 실시간으로 데이터에 접근, 특수 기능 수행, 정확성 향상 등 LLM 과 외부 도구 연동이 필요하다.
    ...
    - 이 때 LLM은 사용자 질의에서 Tool 로 보낼 `arguments` 를 뽑아내서 전달하고 해당 _==**Tool은 arguments 를 통해 기능을 수행 후 결과를 LLM 에 return**==_ 한다.
