...
- 이번 예제에서는 `deepseek-r1` 모델 중 1.5b 짜리를 사용한다.
    ...
    > 위 숫자들은 해당 **==모델이 학습한 파라미터의 갯수를 뜻한다.  
    > ==**671b 은 약 6710억개의 파라미터이므로 용량이 400GB 정도이다.
...
> 참고로 환경변수 키값이 `OPENAI_API_KEY` 인 경우는 **==ChatGPT 기준==**이므로 다른 유료 LLM 모델을 사용한다면 키값이 달라질 수 있다. **==공식문서를 확인해보면 됨!==**
